package graphAndAlgorithms.matrix;

import graphAndAlgorithms.*;
...
public class MatrixGraph<V,E> extends CommonGraph<V,E> {
    ...
    public Collection<Edge<V, E>> incomingEdges(V vert) {
        Collection <Edge<V, E>> incomingEdgesList = new ArrayList<>();

        // get the index of the vertex (since we interpretate the graph as a matrix)
        int vertKey = key(vert);

        // if the vertex is not in the graph, return empty list
        if (vertKey == -1)
            return incomingEdgesList;

        // else we will iterate through the lines of the matrix in the vertKey column and if it has value it means there is an edge to a vertex
        for (int i = 0; i < numVerts; i++)
            if (edgeMatrix[i][vertKey] != null)
                // so we add it to the list
                incomingEdgesList.add(edgeMatrix[i][vertKey]);
